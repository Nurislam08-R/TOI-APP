...
export function GoogleAuth({ onAuthenticated, onBack }: GoogleAuthProps) {
  ...
  const handleGoogleSignIn = () => {
    ...
    setTimeout(() => {
      ...
      onAuthenticated('example@gmail.com', 'User');
    }, 1500);
  };
  ...
  const handleEmailSignIn = () => {
    ...
    if (!email.includes('@gmail.com')) {
      setError('Используйте Gmail адрес');
      return;
    }
      ...
      <div className="flex-1 flex flex-col">
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground">
            Используйте свой Gmail аккаунт
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            <div>
              <label className="block mb-2 text-foreground">Gmail адрес</label>
            ...
            <Button
              ...
              className="w-full bg-gradient-to-r from-accent to-[#B88A16] hover:from-accent/90 hover:to-[#B88A16]/90 text-white py-6 rounded-2xl shadow-lg disabled:opacity-50"
            >
              Продолжить
            </Button>
          </div>
          ...
          <p className="text-center text-muted-foreground mt-6">
            Для демо используйте любой Gmail адрес
          </p>
        </div>
      </div>
    </div>
